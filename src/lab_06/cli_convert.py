...
def main():
    ...
    json_2_csv_parser.add_argument(
        "--input", required=True, help="Ввод директории входного файла"
    )
    json_2_csv_parser.add_argument(
        "--output", required=True, help="Ввод директории файла выхода"
    )

    csv_2_json_parser = subparsers.add_parser("csv2json", help="Перевод из json в csv")
    csv_2_json_parser.add_argument(
        "--input", required=True, help="Ввод директории входного файла"
    )
    csv_2_json_parser.add_argument(
        "--output", required=True, help="Ввод директории файла выхода"
